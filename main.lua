...
local HOTBAR_URL = "https://raw.githubusercontent.com/voquochung2369-creator/mainhub.lua/refs/heads/main/Hotbar.lua"
local LOAD_ANIMATION_URL = "https://raw.githubusercontent.com/voquochung2369-creator/mainhub.lua/refs/heads/main/LoadScriptAnimation.lua"
...
local function LoadRemoteScript(url)
    if type(url) ~= "string" or url == "" or url:find("YOUR_RAW") then
        warn("CustomHub: URL chưa được cấu hình:", url)
        return false
    end
    ...
    if not success then
        warn("CustomHub Load Error:", result)
    end
...
--------------------------------------------------
-- HIDE HUB BUTTON DURING LOAD ANIMATION
--------------------------------------------------
...
MenuContainer.Size = UDim2.new(0, 700, 0, 430)
...
local NormalSize = UDim2.new(0, 650, 0, 400)
local NormalPosition = UDim2.new(0.5, -325, 0.5, -200)

local FullscreenSize = UDim2.new(1, -20, 1, -20)
local FullscreenPosition = UDim2.new(0, 10, 0, 10)
...
MenuTitle.Size = UDim2.new(1, -165, 1, 0)
...
MinimizeButton.Position = UDim2.new(1, -125, 0, 7)
...
FullscreenButton.Parent = MenuTopBar
FullscreenButton.Size = UDim2.new(0, 35, 0, 35)
FullscreenButton.Position = UDim2.new(1, -85, 0, 7)
FullscreenButton.BackgroundColor3 = Color3.fromRGB(100, 100, 100)
FullscreenButton.BorderSizePixel = 0
FullscreenButton.Text = "□"
FullscreenButton.TextColor3 = Color3.fromRGB(255, 255, 255)
FullscreenButton.TextStrokeTransparency = 1
...
FullscreenCorner.CornerRadius = UDim.new(0, 8)
FullscreenCorner.Parent = FullscreenButton

local FullscreenStroke = Instance.new("UIStroke")
FullscreenStroke.Color = Color3.fromRGB(0, 0, 0)
FullscreenStroke.Thickness = 1
FullscreenStroke.Parent = FullscreenButton
...
HotbarContent.ScrollingEnabled = false
...
MainContent.ScrollingEnabled = false
...
local function UpdateMainScroll()
    task.defer(function()
        ...
        local viewportHeight = MainContent.AbsoluteSize.Y
        local needsScroll = contentHeight > viewportHeight + 1

        MainContent.ScrollingEnabled = needsScroll

        if needsScroll then
            MainContent.ScrollBarThickness = 4
            MainContent.CanvasSize = UDim2.new(0, 0, 0, contentHeight + 10)
        else
            MainContent.ScrollBarThickness = 0
            MainContent.CanvasSize = UDim2.new(0, 0, 0, viewportHeight)
...
local function UpdateHotbarScroll()
    task.defer(function()
        local contentHeight = HotbarLayout.AbsoluteContentSize.Y
        local viewportHeight = HotbarContent.AbsoluteSize.Y
        local needsScroll = contentHeight > viewportHeight + 1

        HotbarContent.ScrollingEnabled = needsScroll

        if needsScroll then
            HotbarContent.ScrollBarThickness = 3
            HotbarContent.CanvasSize = UDim2.new(0, 0, 0, contentHeight + 10)
        else
            HotbarContent.ScrollBarThickness = 0
            HotbarContent.CanvasSize = UDim2.new(0, 0, 0, viewportHeight)
            HotbarContent.CanvasPosition = Vector2.new(0, 0)
        end
    end)
end
...
HotbarLayout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
    UpdateHotbarScroll()
end)

HotbarContent:GetPropertyChangedSignal("AbsoluteSize"):Connect(function()
    UpdateHotbarScroll()
end)
...
local function IsNameSelected(list, name)
...
local function RemoveSelectedName(list, name)
    for i = #list, 1, -1 do
        if list[i] == name then
            table.remove(list, i)
            return true
...
local function FindOption(options, name)
    for _, option in ipairs(options) do
        if type(option) == "table" and option.Name == name then
            return option
        end
    end
...
local function SelectTab(tab)
    ...
    for _, data in ipairs(tab.Buttons) do
        ...
        elseif data.Click == "Scroll" then
            ...
            root.Name = data.Name
            root.Parent = MainContent
            root.LayoutOrder = data.Slot
            root.BackgroundTransparency = 1
            root.BorderSizePixel = 0
            root.ClipsDescendants = true

            local function GetExpandedHeight()
                local height = 45 + 150 + 10

                if data.ChildClick == "Button" or data.ChildClick == "Lever" then
                    height = height + 45
                end
            ...
            if expanded then
                root.Size = UDim2.new(1, 0, 0, GetExpandedHeight())
            else
                root.Size = UDim2.new(1, 0, 0, 45)
            end
            ...
            headerStroke.Color = Color3.fromRGB(120, 120, 120)
            ...
            countLabel.Position = UDim2.new(1, -125, 0, 0)
            ...
            scrollFrame.BackgroundColor3 = Color3.fromRGB(75, 75, 75)
            ...
            local function UpdateCount()
                if data.Max then
                    countLabel.Text = tostring(#data.Selected) .. "/" .. tostring(data.Max)
                else
                    countLabel.Text = tostring(#data.Selected) .. " selected"
                end
            end
            ...
            local function SaveScroll()
                if data.Save then
                    SetSavedValue(data.SaveKey, data.Selected)
                end
            end
            ...
            for index, optionData in ipairs(data.Options) do
                ...
                if type(optionData) == "table" then
                    local optionName = tostring(optionData.Name or ("Option " .. index))

                    local option = Instance.new("TextButton")
                    option.Parent = scrollFrame
                    option.Name = optionName
                    option.LayoutOrder = index
                    option.Size = UDim2.new(1, -8, 0, 38)
                    option.BackgroundColor3 = Color3.fromRGB(85, 85, 85)
                    ...
                    local selectedMark = Instance.new("TextLabel")
                    selectedMark.Parent = option
                    selectedMark.Size = UDim2.new(0, 35, 1, 0)
                    selectedMark.Position = UDim2.new(1, -40, 0, 0)
                    selectedMark.BackgroundTransparency = 1
                    selectedMark.TextColor3 = Color3.fromRGB(255, 255, 255)
                    selectedMark.TextStrokeTransparency = 1
                    selectedMark.TextSize = 18
                    selectedMark.Font = Enum.Font.GothamBold
                    selectedMark.TextXAlignment = Enum.TextXAlignment.Center

                    local function UpdateOption()
                        if IsNameSelected(data.Selected, optionName) then
                            option.BackgroundColor3 = Color3.fromRGB(0, 120, 0)
                            selectedMark.Text = "✓"
                        else
                            option.BackgroundColor3 = Color3.fromRGB(85, 85, 85)
                            selectedMark.Text = ""
                    ...
                    option.MouseButton1Click:Connect(function()
                        local selected = IsNameSelected(data.Selected, optionName)

                        if selected then
                            if data.Min and #data.Selected <= data.Min then
                                return
                            end

                            RemoveSelectedName(data.Selected, optionName)
                        else
                            if data.Max and #data.Selected >= data.Max then
                                return
                            end

                            table.insert(data.Selected, optionName)
                        end
            ...
            local function UpdateInnerScroll()
                ...
                if needsScroll then
                    ...
                    scrollFrame.CanvasSize = UDim2.new(0, 0, 0, viewportHeight)
                    scrollFrame.CanvasPosition = Vector2.new(0, 0)
                end
            end

            optionLayout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(UpdateInnerScroll)
            scrollFrame:GetPropertyChangedSignal("AbsoluteSize"):Connect(UpdateInnerScroll)
            ...
            if data.ChildClick == "Button" then
                childControl = Instance.new("TextButton")
                childControl.Parent = root
                childControl.Size = UDim2.new(1, 0, 0, 40)
                childControl.Position = UDim2.new(0, 0, 0, 210)
                childControl.BackgroundColor3 = Color3.fromRGB(100, 100, 100)
                childControl.BorderSizePixel = 0
                childControl.Text = data.ChildName or "Execute Selected"
                childControl.TextColor3 = Color3.fromRGB(255, 255, 255)
                childControl.TextStrokeTransparency = 1
                childControl.TextSize = 14
                childControl.Font = Enum.Font.GothamBold
                childControl.Visible = expanded

                local childCorner = Instance.new("UICorner")
                childCorner.CornerRadius = UDim.new(0, 8)
                childCorner.Parent = childControl

                local childStroke = Instance.new("UIStroke")
                childStroke.Color = Color3.fromRGB(120, 120, 120)
                childStroke.Thickness = 1
                childStroke.Parent = childControl

                childControl.MouseButton1Click:Connect(function()
                    if data.Min and #data.Selected < data.Min then
                        return
                    end
                    ...
                    for _, selectedName in ipairs(data.Selected) do
                        local option = FindOption(data.Options, selectedName)

                        if option and typeof(option.Callback) == "function" then
                            task.spawn(function()
                                pcall(function()
                                    option.Callback(selectedName, data.Selected)
                                end)
                            end)
                        end
                    end
            ...
            elseif data.ChildClick == "Lever" then
                ...
                childControl.BackgroundColor3 = Color3.fromRGB(100, 100, 100)
                childControl.BorderSizePixel = 0
                childControl.Text = "   " .. (data.ChildName or "Auto Execute")
                childControl.TextColor3 = Color3.fromRGB(255, 255, 255)
                childControl.TextStrokeTransparency = 1
                childControl.TextSize = 14
                childControl.Font = Enum.Font.GothamBold
                childControl.TextXAlignment = Enum.TextXAlignment.Left
                childControl.Visible = expanded
                ...
                local childLever = Instance.new("Frame")
                childLever.Parent = childControl
                childLever.Size = UDim2.new(0, 65, 0, 28)
                childLever.Position = UDim2.new(1, -75, 0.5, -14)
                childLever.BorderSizePixel = 0

                local childLeverCorner = Instance.new("UICorner")
                childLeverCorner.CornerRadius = UDim.new(1, 0)
                childLeverCorner.Parent = childLever

                local childLeverText = Instance.new("TextLabel")
                childLeverText.Parent = childLever
                childLeverText.Size = UDim2.new(1, 0, 1, 0)
                childLeverText.BackgroundTransparency = 1
                childLeverText.TextSize = 12
                childLeverText.Font = Enum.Font.GothamBold
                childLeverText.TextColor3 = Color3.fromRGB(255, 255, 255)
                childLeverText.TextXAlignment = Enum.TextXAlignment.Center

                local childCircle = Instance.new("Frame")
                childCircle.Parent = childLever
                childCircle.Size = UDim2.new(0, 22, 0, 22)
                childCircle.BorderSizePixel = 0
                childCircle.BackgroundColor3 = Color3.fromRGB(255, 255, 255)

                local childCircleCorner = Instance.new("UICorner")
                childCircleCorner.CornerRadius = UDim.new(1, 0)
                childCircleCorner.Parent = childCircle

                local function UpdateChildLever()
                    if data.ChildEnabled then
                        childLever.BackgroundColor3 = Color3.fromRGB(0, 150, 0)
                        childLeverText.Text = "ON"
                        childCircle.Position = UDim2.new(1, -25, 0.5, -11)
                    else
                        childLever.BackgroundColor3 = Color3.fromRGB(70, 70, 70)
                        childLeverText.Text = "OFF"
                        childCircle.Position = UDim2.new(0, 3, 0.5, -11)
                    end
                end
                ...
                childControl.MouseButton1Click:Connect(function()
                    ...
                    data.ChildEnabled = not data.ChildEnabled

                    if data.ChildSave then
                        SetSavedValue(data.ChildSaveKey, data.ChildEnabled)
                    end

                    UpdateChildLever()

                    if typeof(data.ChildCallback) == "function" then
                        task.spawn(function()
                            pcall(function()
                                data.ChildCallback(data.ChildEnabled, data.Selected)
                            end)
                        end)
                    end
                end)

                if data.ChildEnabled and #data.Selected > 0 and typeof(data.ChildCallback) == "function" then
                    task.spawn(function()
                        pcall(function()
                            data.ChildCallback(true, data.Selected)
                        end)
                    end)
                end
            end
...
function Window:MakeTab(data)
    ...
    local tab = {
        ...
        Slot = tonumber(data.Slot) or (#Window.Tabs + 1),
        Link = data.Link,
        Loaded = false,
        Buttons = {},
        Sequence = 0
    ...
    tabButton.LayoutOrder = tab.Slot
    ...
    tabButton.MouseButton1Click:Connect(function()
        ...
        if tab.Link and not tab.Loaded then
            tab.Loaded = true

            task.spawn(function()
                LoadRemoteScript(tab.Link)
    ...
    function tab:AddButton(buttonData)
        ...
        local info = {
            ...
            Slot = tonumber(buttonData.Slot) or tab.Sequence,
            Sequence = tab.Sequence
        }

        if info.Click ~= "Button" and info.Click ~= "Lever" and info.Click ~= "Scroll" then
            info.Click = "Button"
        end
        ...
        if info.Click == "Button" then
            info.SaveKey = tab.Name .. "_" .. tostring(info.Slot) .. "_" .. tostring(info.Name)
        ...
        elseif info.Click == "Scroll" then
            info.Min = tonumber(buttonData.Min)
            info.Max = tonumber(buttonData.Max)

            info.Classic = {}

            if type(buttonData.Classic) == "table" then
                for _, value in ipairs(buttonData.Classic) do
                    if type(value) == "string" then
                        table.insert(info.Classic, value)
                    end
                end
            end

            info.Options = type(buttonData.Options) == "table" and buttonData.Options or {}

            info.SaveKey = tab.Name .. "_" .. tostring(info.Slot) .. "_" .. tostring(info.Name) .. "_Scroll"
            info.Selected = {}

            local saved = GetSavedValue(info.SaveKey)

            if info.Save and type(saved) == "table" then
                for _, savedName in ipairs(saved) do
                    if FindOption(info.Options, savedName) then
                        table.insert(info.Selected, savedName)
                    end
                end
            else
                for _, classicName in ipairs(info.Classic) do
                    if FindOption(info.Options, classicName) then
                        table.insert(info.Selected, classicName)
                    end
                end
            end

            if info.Max and #info.Selected > info.Max then
                while #info.Selected > info.Max do
                    table.remove(info.Selected)
                end
            end

            info.ChildClick = buttonData.ChildClick
            info.ChildName = buttonData.ChildName
            info.ChildDefault = buttonData.ChildDefault == true
            info.ChildSave = buttonData.ChildSave == true or info.Save
            info.ChildCallback = buttonData.ChildCallback

            info.ChildSaveKey = tab.Name .. "_" .. tostring(info.Slot) .. "_" .. tostring(info.Name) .. "_Child"
            info.ChildEnabled = info.ChildDefault

            local childSaved = GetSavedValue(info.ChildSaveKey)

            if info.ChildSave and childSaved ~= nil then
                info.ChildEnabled = childSaved
            end
...
FullscreenButton.MouseButton1Click:Connect(function()
    ...
    if IsFullscreen then
        MenuContainer.Size = FullscreenSize
        MenuContainer.Position = FullscreenPosition
        FullscreenButton.Text = "❐"
    else
        MenuContainer.Size = NormalSize
        MenuContainer.Position = NormalPosition
        FullscreenButton.Text = "□"
    end
...
--------------------------------------------------
-- LOAD SCRIPT ANIMATION
--------------------------------------------------

task.spawn(function()
    local success, result = LoadRemoteScript(LOAD_ANIMATION_URL)

    if not success then
        warn("[CustomHub] LoadScriptAnimation Error:", result)

        if HubButton and HubButton.Parent then
